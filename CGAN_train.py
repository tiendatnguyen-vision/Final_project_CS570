...
def load_data(target_size = (IMG_SIZE,IMG_SIZE), img_folder = ""):
    ...
    for pneumonial_type in pneunomial_classes:
        ...
        for imgname in imgnames:
            ...
            X.append(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
...
def generate_image(epoch, save_dir, generator, n_classes=NUM_CLASSES, latent_dim = 100):
    ...
    images = generator.predict([z_input, labels]) # range = [-0.99999475, 0.92242205]
    images = (images+1)/2.0
...
dataset = load_data(target_size=(IMG_SIZE,IMG_SIZE), img_folder="Pneumonia_condition") # dataset in range(-1,1)
